...
- **`full.yml`** — виртуальная периферия (`vcan`/`i2c-stub`/`gpio-sim`) и тестовый MQTT-стенд.
  Планируется запускать по тегу `full-*` на любом коммите без создания ветки:
  `git tag full-1 <sha> && git push origin full-1`. Требует `emulator/tests/test_devices.cpp` и
  `emulator/stand/compose.yml`, которых пока нет — не коммитил `full.yml`, который сразу падал бы.
